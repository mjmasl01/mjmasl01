...
In the context of the problem, the fitted regression coefficient for age is 0.013. This means that for every one year increase in age of the patient, the log-odds of having a heart disease increases by 0.013, given all other predictors are constant.
...
```{r}
heart_df %>%
  mutate(target = ifelse(target == 1, "Heart Disease", "No Heart Disease"),
         sex = ifelse(sex == 1, "Male", "Female")) %>%
  ggplot(aes(x = age, y = chol, color = sex)) +
    geom_boxplot() +
    facet_wrap(~target) +
    labs(title = "Predicted Probability of Heart Disease by Age and Cholesterol",
         x = "Age",
         y = "Cholesterol",
         color = "Gender",
         shape = "Target") +
