...
df_major %>% filter(discipline == "STAT") %>% group_by(sex) %>% summarise(n_students = n())
  ...
  sidebarLayout(
    sidebarPanel(selectInput("major_choice",
                 ...
                 sliderInput("n_majors",
                             ...
                             min = 1, max = 15,
                             value = 1, step = 1),
                 radioButtons("major_minor",
                              label = "Choose Major or Minor",
...
server <- function(input, output, session) {
  ...
  major_react <- reactive({
    ...
    df_statfull <- semi_join(df_long, df_stat, by = "adm_id") |>
      filter(str_detect(type, pattern = input$major_minor) == TRUE)
  ...
  output$sex_table <- renderTable({
    df_major %>% filter(discipline == input$major_choice) %>% group_by(sex) %>% summarise(n_students = n())
  })
